...
document.addEventListener("DOMContentLoaded", () => {
      ...
      basicForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          for (const factor of ["D", "O", "G", "L", "B"]) {
            ...
            // Add explanation as caption if available
    ...
    if (advancedForm) {
      advancedForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          // Call the Gemini API with parameters
          const factors = await callGeminiAPI({
            leftoverIncome,
          ...
          // Render factor cards with explanations
  ...
  function renderItemCard(itemName, cost) {
    // Choose icon based on price and name
    let icon = "🛍️";
    if (cost >= 5000) icon = "💰";
  ...
  function renderFactorCard(factor, value, description) {
    // Enhanced color logic for factor value
